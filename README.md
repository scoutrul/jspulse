...
- Если VS Code/Cursor показывает ошибки типов после изменений, попробуйте:
  1. Пересобрать `shared` пакет: `pnpm run build`
  2. Перезапустить TypeScript сервер в редакторе
  3. При необходимости перезапустить сервер разработки: `pnpm run dev`
...
Пакет `shared` содержит типизацию данных, используемую во всем проекте. Важно понимать, что исходные файлы TypeScript компилируются в JavaScript и декларации типов (`.d.ts`), которые размещаются в директории `dist/`. Это необходимо для корректной работы с локальными зависимостями в монорепозитории.
...
- Директория `dist/` содержит скомпилированные файлы и декларации типов
- НЕ включается в Git (см. `.gitignore`)
- Генерируется автоматически при сборке (`pnpm run build`)
- Необходима для корректной работы импортов между пакетами в монорепозитории
- При изменении типов требуется пересборка (`pnpm run build` в корне)
