...
- **Внедрен сервисный слой на фронтенде:**
  - Создан модуль `services` с отдельными сервисами для клиентской и серверной частей
  - Вынесены функции трансформации данных в отдельный модуль `utils/vacancyTransformations.ts`
  - Устранено дублирование кода между клиентом и сервером
  - Улучшена типизация и обработка ошибок
- **Реализована полноценная функциональность очистки базы данных:**
  - Доработан скрипт `clearDatabase.ts` для удаления всех вакансий
  - Добавлен вывод статистики по количеству удаленных записей
...
- Упрощен процесс разработки: удалены хуки husky, вызывающие ошибки при коммитах.
- Исправлена ошибка в корневом package.json (изменено имя пакета с `@jspulse/shared` на `jspulse`).
- Исправлен Dockerfile (удален флаг `--frozen-lockfile`) для стабильной сборки в Docker.
